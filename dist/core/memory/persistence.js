...
class PersistenceLayer {
    ...
    static async openForRepo(repoRoot) {
        const migrated = await (0, folders_1.migrateLegacyIfNeeded)(repoRoot);
        if (migrated.migrated) {
            process.stderr.write(`Warning: migrated legacy state folder "${migrated.from}" to "${migrated.to}".\n`);
        }
    ...
    listTasksForRun(args) {
        const rows = this.db
            .prepare(`SELECT task_id as taskId, status, phase, iteration, last_error as lastError
         FROM tasks
         WHERE run_id=@runId
