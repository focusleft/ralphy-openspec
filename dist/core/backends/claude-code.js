    ...
    async implement(env, input) {
        ...
        try {
            ...
            if (timedOut || (result.exitCode === 143 && !result.stdout && !result.stderr)) {
                ...
                return {
                    ok: false,
                    message: `Claude Code timed out after ${timeoutMinutes} minute(s). The task budget allows ${task.budget?.hard?.timeMinutes ?? "N/A"} minutes. ${task.budget?.hard?.timeMinutes && taskTimeoutMs >= task.budget.hard.timeMinutes * 60_000
