...
class CursorBackend {
    ...
    async implement(env, input) {
        ...
        const taskTimeoutMs = task.budget?.hard?.timeMinutes !== undefined
        ...
        if (env.logFile) {
            try {
                await promises_1.default.mkdir(node_path_1.default.dirname(env.logFile), { recursive: true });
                logFileHandle = await promises_1.default.open(env.logFile, "w");
                // Write header immediately
                await logFileHandle.write(`# Backend log (real-time)\n\n- startedAt: ${startedAt}\n- backend: ${env.backendId}\n- cwd: ${env.cwd}\n- command: ${command}\n- argv: ${JSON.stringify(argv.map((a, i) => (i === argv.length - 1 ? "<prompt redacted>" : a)))}\n- timeoutMs: ${taskTimeoutMs} (${Math.floor(taskTimeoutMs / 60_000)} minutes)\n\n## Real-time output\n\n`);
            }
            catch {
                logFileHandle = null;
            }
        ...
        const appendLog = async (text, stream = "stdout") => {
            if (logFileHandle) {
                try {
        ...
        try {
             ...
             */
            const subprocess = (0, execa_1.execa)(command, argv, {
                cwd: env.cwd,
            ...
            if (subprocess.stderr) {
                subprocess.stderr.on("data", async (chunk) => {
                    const text = chunk.toString();
                    stderr += text;
            ...
            if (env.logFile && !logWritten) {
                try {
                    await (0, log_writer_1.writeBackendLog)({
                        logFile: env.logFile,
        ...
        const timedOut = result ? result.timedOut === true : false;
        ...
        if (timedOut || (exitCode === 143 && !combined)) {
            ...
            return {
                ok: false,
                message: `Cursor Agent timed out after ${timeoutMinutes} minute(s). The task budget allows ${task.budget?.hard?.timeMinutes ?? "N/A"} minutes. ${task.budget?.hard?.timeMinutes && taskTimeoutMs >= task.budget.hard.timeMinutes * 60_000
        ...
        return {
            ok: false,
            message: `Cursor Agent exited with code ${exitCode}: ${combined || "(no output)"}${exitCode === 143
