...
class EngineLoop {
    async run(opts) {
        ...
        progress(`Run started: ${runId} (backend=${opts.backend.id}, workspace=${opts.workspace.mode})`);
    ...
    async runOneTask(args) {
        ...
        for (let iter = 1; iter <= maxIter; iter++) {
            ...
            const backendLogFile = artifacts
                ? node_path_1.default.join((0, folders_1.getRalphyRoot)(args.repoRoot, artifacts.rootDir), folders_1.FOLDERS.logs, runId, `${task.id}_iter-${iter}_${args.backend.id}.md`)
                : undefined;
            if (backendLogFile) {
                await promises_1.default.mkdir(node_path_1.default.dirname(backendLogFile), { recursive: true });
                args.progress?.(`Backend log: ${node_path_1.default.relative(args.repoRoot, backendLogFile)}`);
            }
            ...
            const heartbeat = args.progress
                ? setInterval(() => {
                    ...
                    args.progress?.(`Backend still running: ${task.id} iteration ${iter} (elapsed=${elapsedSec}s)`);
                }, 30_000)
                : null;
